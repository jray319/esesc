...
BCache::BCache(MemorySystem *gms, const char *section, const char *name)
	: MemObj(section, name)
	, reqFromUpPort(0)
	, respFromUpPort(0)
	, fromDownPort(0)
	...
	, upNodeNum(SescConf->getInt(section, "upNodeNum"))
{
  ...
  const char* mshrSection = SescConf->getCharPtr(section,"MSHR");
	...
	SescConf->isInt(mshrSection, "upSizePerBank");
	SescConf->isInt(mshrSection, "downSizePerBank");
...
void BCache::req(MemRequest *mreq) {
	I(mreq);
	ID(mreq->dump("req"));
...
void BCache::reqAck(MemRequest *mreq) {
	I(mreq);
	ID(mreq->dump("reqAck"));
	I(!mreq->isRetrying());
	// [sizhuo] enq msg to inport & record inport & set pos
...
void BCache::setState(MemRequest *mreq) {
	I(mreq);
...
void BCache::setStateAck(MemRequest *mreq) {
	I(mreq);
...
void BCache::disp(MemRequest *mreq) {
	I(mreq);
	ID(mreq->dump("disp"));
	I(!mreq->isRetrying());
	// [sizhuo] enq new msg to inport
...
void BCache::doReq(MemRequest *mreq) {
	I(mreq);
	ID(mreq->dump("doReq"));
...
void BCache::doReqAck(MemRequest *mreq) {
	I(mreq);
...
void BCache::doSetState(MemRequest *mreq) {
	I(mreq);
...
void BCache::doSetStateAck(MemRequest *mreq) {
	I(mreq);
...
void BCache::doDisp(MemRequest *mreq) {
	I(mreq);
	ID(mreq->dump("doDisp"));
...
TimeDelta_t BCache::ffread(AddrType addr) {
	return tagDelay + dataDelay;
}

TimeDelta_t BCache::ffwrite(AddrType addr) {
	return tagDelay + 1;
}

bool BCache::isBusy(AddrType addr) const {
	return false;
}
